...
To avoid universe issues and make a compactness argument easier later on, we express this using
measures on `G`, but the equivalence with the above point of view follows from `rhoMinus_le`. -/
...
lemma nonneg_of_mem_rhoMinusSet {Ω : Type*} [MeasureSpace Ω] [IsProbabilityMeasure (ℙ : Measure Ω)]
  ...
  exact KL_div_nonneg hX (by fun_prop) habs
...
lemma rhoMinus_le {Ω : Type*} [MeasureSpace Ω] [IsProbabilityMeasure (ℙ : Measure Ω)]
  ...
  exact ⟨volume.map T, isProbabilityMeasure_map hT.aemeasurable, by rwa [M], by simp [KL_div, M]⟩

/-- For any $G$-valued random variable $X$, we define $\rho^+(X) := \rho^-(X) + \bbH(X) - \bbH(U_A)$. -/
noncomputable def rhoPlus {Ω : Type*} [MeasureSpace Ω]
    (X : Ω → G) (A : Finset G) : ℝ := (rhoMinus X A) + H[ X ] - Real.log (Nat.card A)
...
private lemma le_rhoMinus_of_subgroup (H : AddSubgroup G) {Ω : Type*} [MeasureSpace Ω]
    ...
    Real.log (Nat.card A) -
      Real.log (sSup {Nat.card (A ∩ (t +ᵥ (H : Set G)) : Set G) | t : G} : ℕ) ≤ rhoMinus U A := by
  ...
  have I₁ : KL[U # T + UA] =
      ∑ h ∈ H', 1/Nat.card H * Real.log ((1/Nat.card H) / (volume.map (T + UA) {h}).toReal) := by
    rw [KL_div_eq_sum, ← Finset.sum_subset (Finset.subset_univ H')]; swap
    · intro x _ hH
    ...
    congr <;> simp [H']
  have I₂ : (∑ h ∈ H', 1/Nat.card H : ℝ) * Real.log ((∑ h ∈ H', 1/Nat.card H : ℝ)
  ...
  simp only [Finset.sum_toReal_measure_singleton, one_div, Real.log_inv] at I₂
  apply le_trans _ I₂
  ...
  rw [one_mul] at I₃
  have : - Real.log ((sSup {Nat.card (A ∩ (t +ᵥ (H : Set G)) : Set G) | t : G}) / Nat.card A) ≤
      - Real.log ((Measure.map (T + UA) ℙ) ↑H').toReal := by
    apply neg_le_neg
    apply Real.log_le_log _ I₃
...
private lemma rhoMinus_le_of_subgroup {H : AddSubgroup G} {Ω : Type*} [MeasureSpace Ω]
    ...
    (hU : Measurable U) :
    rhoMinus U A ≤ Real.log (Nat.card A) -
      Real.log (Nat.card (A ∩ (t +ᵥ (H : Set G)) : Set G)) := by
  ...
  have Imp y (h : (Measure.map (Prod.fst + Prod.snd) (μ.prod (uniformOn ↑A))) {y} = 0) : y ∉ H := by
    intro yH
  ...
  · rw [mapU]
    exact uniformOn_apply_singleton_of_nmem (fun yH ↦ Imp _ h yH)
  let H' : Finset G := Set.Finite.toFinset (toFinite H)
  have hunif' : IsUniform H' U := by convert hunif; simp [H']
  rw [KL_div_eq_sum, ← Finset.sum_subset (Finset.subset_univ H')]; swap
  · intro x _ hH
  ...
  have : ∑ x ∈ H', ((Measure.map U ℙ) {x}).toReal *
      Real.log (((Measure.map U ℙ) {x}).toReal
        / ((Measure.map (Prod.fst + Prod.snd) (μ.prod (uniformOn ↑A))) {x}).toReal)
      = ∑ x ∈ H', (1/Nat.card H) * Real.log ((1/Nat.card H)
...
lemma rhoMinus_of_subgroup (H : AddSubgroup G) {Ω : Type*} [MeasureSpace Ω]
    ...
    (U : Ω → G) (hunif : IsUniform H U) {A : Finset G} (hA : A.Nonempty) (hU : Measurable U) :
    rhoMinus U A = Real.log (Nat.card A) -
      Real.log (sSup {Nat.card (A ∩ (t +ᵥ (H : Set G)) : Set G) | t : G} : ℕ) := by
...
lemma rhoPlus_of_subgroup (H : AddSubgroup G) {Ω : Type*} [MeasureSpace Ω]
    [IsProbabilityMeasure (ℙ : Measure Ω)]
    (U : Ω → G) (hunif : IsUniform H U) {A : Finset G} (hA : A.Nonempty) (hU : Measurable U) :
    rhoPlus U A = Real.log (Nat.card H) -
      Real.log (sSup {Nat.card (A ∩ (t +ᵥ (H : Set G)) : Set G) | t : G} : ℕ) := by
  have : H[U] = Real.log (Nat.card H) := hunif.entropy_eq' (toFinite _) hU
...
lemma rho_of_subgroup (H: AddSubgroup G) {Ω : Type*} [MeasureSpace Ω]
    ...
    (r : ℝ) (hr : rho U A ≤ r) :
    ∃ t : G,
      Nat.card (A ∩ (t +ᵥ (H : Set G)) : Set G) ≤ Real.exp (-r) * (Nat.card A * Nat.card H)^(1/2) ∧
        Nat.card A ≤ Real.exp (2 * r) * Nat.card H ∧
        Nat.card H ≤ Real.exp (2 * r) * Nat.card A := by
  simp [rho] at hr
